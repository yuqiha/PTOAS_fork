...
enum DtypeKind { DK_F16_BF16 = 0, DK_F32_F32 = 1 };
...
static int RunCase(const TestCase &tc, int deviceId, aclrtStream stream) {
    (void)deviceId;
    int rc = 0;
    size_t lhsBytes = tc.M * tc.K * tc.dtype_a_size;
    size_t rhsBytes = tc.K * tc.N * tc.dtype_b_size;
    size_t outBytes = tc.M * tc.N * tc.dtype_c_size;
