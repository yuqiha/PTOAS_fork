...
"""TileLang DSL template for pto.trowexpandmin"""
...
def template_trowexpandmin(src0: pto.Tile, src1: pto.Tile, dst: pto.Tile):
    ...
    Compute element-wise min of each row of src0 with a per-row scalar from src1[row, 0].
    Semantics: dst[row, col] = min(src0[row, col], src1[row, 0])
    """
