...
@pto.vkernel(
    ...
    constraints=[_validate_tcolargmax],
    advanced=True,
)
def template_tcolargmax_i8_to_i32(src: pto.Tile, tmp: pto.Tile, dst: pto.Tile):
    ...
    with pto.vecscope():
        ...
        for col in range(0, src_valid_cols, lanes_i8):
            ...
            for row in range(1, src_valid_rows, 1):
                ...
                select_even = pto.vcmp(vreg_new_even, vreg_old_even, all_mask_intermediate, "gt")
                select_odd = pto.vcmp(vreg_new_odd, vreg_old_odd, all_mask_intermediate, "gt")
                ...
                vreg_old_even = pto.vmax(vreg_old_even, vreg_new_even, all_mask_intermediate)
                vreg_old_odd = pto.vmax(vreg_old_odd, vreg_new_odd, all_mask_intermediate)
...
def template_tcolargmax_f16_to_i32(src: pto.Tile, tmp: pto.Tile, dst: pto.Tile):
    src_valid_rows, src_valid_cols = src.valid_shape
    ...
    with pto.vecscope():
        ...
        for col in range(0, src_valid_cols, lanes_f16):
            ...
            max_vals = pto.vlds(src[0, col:])

            for row in range(1, src_valid_rows, 1):
                index_new = pto.vadds(index_new, pto.i16(1), mask_f16)
                new_vals = pto.vlds(src[row, col:])
                gt_mask = pto.vcmp(new_vals, max_vals, all_mask, "gt")
                index_old = pto.vsel(index_new, index_old, gt_mask)
                max_vals = pto.vmax(max_vals, new_vals, mask_f16)
...
def template_tcolargmax_f32_to_i32(src: pto.Tile, tmp: pto.Tile, dst: pto.Tile):
    src_valid_rows, src_valid_cols = src.valid_shape
    ...
    with pto.vecscope():
        ...
        for col in range(0, src_valid_cols, lanes):
            ...
            for row in range(1, src_valid_rows, 1):
                ...
                gt_mask = pto.vcmp(new_vals, max_vals, all_mask, "gt")
                index_old = pto.vsel(index_new, index_old, gt_mask)
                max_vals = pto.vmax(max_vals, new_vals, mask)
